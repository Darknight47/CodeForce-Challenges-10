...
------------------------------- Link for the challenge: https://codeforces.com/problemset/problem/1901/A -----------------------------
...
for i in range(cases):
    sze, pos = map(int, input().split())
    lst = list(map(int, input().split()))
    minVol = abs(lst[sze - 1] - pos) * 2
    for j in range(sze - 1):
        if(abs(lst[j] - lst[j + 1]) > minVol):
            minVol = abs(lst[j] - lst[j + 1])
    if(abs(0 - lst[0]) > minVol):
        minVol = abs(0 - lst[0])
    print(minVol)